...
datatable(truffles_var, caption = "All variables",
...
truffles %>%
  ...
  datatable(caption = "Truffles data set (n = 30)",
...
```{r scatter-truffles, fig.width=14, message=FALSE, warning=FALSE}
...
pairs(~P+Q+DI+PS+PF, data=truffles,
...
```{r simple-P,results="asis"}
fun_report_eq(mod_simple$mod.P,lm.dt = truffles)
```
...
```{r simple-Q,results="asis"}
fun_report_eq(mod_simple$mod.Q,lm.dt = truffles)
```
...
```{r scatter-truffles-left, fig.cap="SRL of the Price model",fig.width=6}
ggplot(truffles,aes(x=Q,y=P))+
  geom_point(color="blue") +
...
```{r scatter-truffles-right, fig.cap="SRL of the Quantity model", fig.width=6}
...
cov_yu<-monte %>% mutate(yu=(Y-mean(Y))*(u-mean(u))) %>%summarise(sum(yu)) %>% unlist() 
ss_y<- monte %>% mutate(Y_sqr=(Y-mean(Y))^2) %>% summarise(sum(Y_sqr)) %>% unlist()
...
$\hat{\beta_1}$ 
is different from
$\beta_1=0.8$, and the differnce is `r formatC(cov_yu/ss_y,digits=4,format="f")`.
...
```{r scatter-YC-fit, fig.height= 6.5, fig.cap="The SRL"}
ggplot(monte,aes(x=Y,y=C))+
  geom_point(color="red") +
  geom_smooth(se=FALSE,method = "lm",alpha=0.6,color="grey")+
