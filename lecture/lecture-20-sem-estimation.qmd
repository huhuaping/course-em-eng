...
include_graphics("pic/chpt20-recursive-graph.png", dpi = 200)
```
...
us_agr<-as_tibble(read.xlsx("data/Table-20-1.xlsx"))
n<-dim(us_agr)[1]
datatable(us_agr,
options = list(pageLength =7, dom = 'tip'),
caption =str_c( "sample data (n=",
n,")"))
```
...
#| label: P-agr
#| results: asis
fun_report_eq(models_agr$mod.P,lm.dt = us_agr)
```
...
#| label: Q-agr
#| results: asis
fun_report_eq(models_agr$mod.Q,lm.dt = us_agr)
```
:::
...
#| label: bias-OLS
#| results: asis
fun_report_eq(lm.mod = models_agr$mod.Q.OLS,
lm.dt = us_agr)
```
...
::: {.notes}
.large[两阶段最小二乘法(2SLS方法)的特点(续)：- 此法虽然专为过度识别的方程而设计，但同样适用于恰好识别的方程。但这时ILS和2SLS将给出相同的估计。(为什么?)- 如果约简型回归（即阶段1的回归）的F值很高，比如说高于0.8。则经典OLS估计和 2SLS估计将相差无几。- 在ILS方法的回归报告中，我们没有给出所估系数的**标准误**，但我们能对 2SLS估计值给出这些**标准误**。]
:::
...
#| label: data-money
#truffles<-read.table("data/truffles.dat",header = F)
us_money<-as_tibble(read.xlsx("data/Table-20-2.xlsx")) %>%
mutate(Y1.l1 = dplyr::lag(Y1),
Y2.l1 = dplyr::lag(Y2)
) %>%
select(-X3)
n<-dim(us_money)[1]
...
#| results: asis
#| eval: true
models_money<-list(mod.Y1=Y1~Y2+X1+X2,
mod.Y2=Y2~Y1+X2,
mod.instr=Y1~X1+X2,
mod.stage2=Y2~Y1.hat,
mod.ols=Y2~ Y1)
```
...
mutate(Y1.hat=round(money_out$fitted.values,4),
...
```{r}
#| results: asis
fun_report_eq(models_money$mod.instr,lm.dt = us_money)
```
...
#| label: stage2-reg
#| results: asis
fun_report_eq(models_money$mod.stage2,lm.dt = us_money_new)
```
...
#| results: asis
fun_report_eq(lm.mod = models_money$mod.ols,
lm.dt = us_money_new)
```
...
#| results: asis
#| eval: true
models_money2<-list(mod2.Y1=Y1~Y2+X1+X2,
mod2.Y2=Y2~Y1+Y1.l1+Y2.l1,
mod2.instr.1=Y1~X1+X2+Y1.l1+Y2.l1,
mod2.instr.2=Y2~X1+X2+Y1.l1+Y2.l1,
mod2.stage2.1=Y1~Y2.hat+X1+X2,
mod2.stage2.2=Y2~Y1.hat+Y1.l1+Y2.l1)
...
mutate(Y1.hat=c(NA, money2_instr_out1$fitted.values),
...
fun_report_eq(models_money2$mod2.instr.1,lm.dt = us_money)
...
fun_report_eq(models_money2$mod2.instr.2,lm.dt = us_money)
...
#| results: asis
fun_report_eq(models_money2$mod2.stage2.1,lm.dt = us_money_new2)
```
...
#| results: asis
fun_report_eq(models_money2$mod2.stage2.2,lm.dt = us_money_new2)
```
...
#| comment: ''
tab_income_s2<-lm(models_money2$mod2.stage2.2, data=us_money_new2)
summary(tab_income_s2)
```
...
```{r}
#| label: 2SLS-model-money2
require(systemfit)
sys<-list(eq1=Y1~Y2+X1+X2,
eq2=Y2~Y1+Y1.l1+Y2.l1)
instr <- ~X1+X2+Y1.l1+Y2.l1
money.sys <- systemfit(sys, inst=instr,
...
#| results: asis
fun_report_eq(lm.mod = models_money2$mod2.Y1,
lm.dt = us_money_new2)
```
...
#| results: asis
fun_report_eq(lm.mod = models_money2$mod2.Y2,
lm.dt = us_money_new2)
```
...
datatable(truffles_var, caption = "variables description",
...
### Simple OLS solution: tidy report
...
source("code/helper-tidy-systemfit.R")
...
### Comparison: results of the reduced equations by OLS directly
...
#| label: reduce-P
#| results: asis
fun_report_eq(models$mod.P,lm.dt = truffles)
```
...
#| label: reduce-Q
#| results: asis
fun_report_eq(models$mod.Q,lm.dt = truffles)
```


##

### Comparison: results of the structural equations by OLS directly
...
options = list(pageLength =7, dom = 'tip'),
caption = str_c("Data set of cod case(obs. n=",n,")")) %>%
...
```{r}
ggplot(fultonfish,aes(x=lquan,y=lprice))+
geom_point(color="blue") +
...
```{r}
#| label: fish-reduce-model
models_fish<-list(mod.Q=lquan~mon+tue+wed+thu+stormy,
mod.P=lprice~mon+tue+wed+thu+stormy)
...
tab_smry<-as_tibble(fish.smry$coefficients)
names(tab_smry)<-cnames
tab_smry$vars<-rnames
tab_smry<-tab_smry %>%
separate(vars,into=c("eq","vars"),sep="_")%>%
...
#kable(tab_smry,digits = 4,caption = "两阶段最小二乘法（2SLS）回归结果")
```
