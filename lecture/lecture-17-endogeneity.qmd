...
```{r}
#| results: hide
source(here("lecture/code/code-chapter-17-endogeneity.R"))
...
[17.6 Testing regressor endogeneity](#test-endogeneity)
...
![CLRM Assumptions and Random Experiments](pic/review-CLRM.png){fig-align="center" width="70%"}

::: {.notes}
这张图展示了经典线性回归模型(CLRM)的关键假设和随机试验的概念。左侧列出了CLRM的五个基本假设,右侧说明了随机试验的特点。这有助于学生回顾之前学习的重要概念。
:::
...
- **CLRM A2** assumes that $X$ is fixed (given) or independent of the error term. The regressors $X$ **are not** random variables. At this point, we can use the OLS method and get [**BLUE**]{.blue} (**B**est **L**inear **U**nbiased **E**stimator).
...
- If the above A2 assumption is violated, the independent variable $X$ is related to the random disturbance term. In this case, OLS estimation will no longer get [**BLUE**]{.blue}, and **instrumental variable method** (IV) should be used for estimation.
...
![](pic/fx-endogeneity3.png){fig-align="center" width="70%"}
...
The concept of **endogeneity** is used broadly to describe any situation where a regressor is **correlated** with the error term.
...
- The explanatory variable $X$ is said to be **[En]{.red}dogenous** if it is correlated with $\epsilon$.
...
![](pic/fx-endogeneity-source-omit.png){fig-align="center" height="60%"}

::: {.notes}
生图提示词：

I am a university teacher. I want to generate a picture on the concept of endogeneity on my econometrics class to my students. the picture should be: Simple line drawing style, transparent background. On the left side draw a diploma icon (education variable), right side a money bag (wage). Draw solid arrow from diploma to money bag labeled 'Estimated effect'. Above them draw a brain icon (ability variable) sending dashed arrows to both diploma and money bag, with red cross over the brain. Add warning sign at bottom with text 'Omitted Variable Causes Endogeneity'. All elements in black lines, minimalist academic diagram style.
...
:::{.absolute top=282px left=790px width=150px height=80px style="background-color:#cceffc; font-weight:bold; color:#000; text-align:left; z-index:999;font-size:85%!important;"}
<!-- .box-D4 -->
(Demand
:::
...
::: {.absolute top=398px left=795px width=150px height=80px style="background-color:#cceffc; font-weight:bold; color:#000; text-align:left; z-index:999;font-size:85%!important;"}
<!-- .box-D3 -->
(Supply
:::
...
::: {.absolute top=640px left=770px width=420px height=135px style="background-color:#ffce33; font-weight:bold; color:#000; text-align:center; z-index:999; font-size:85%!important;display: flex;align-items: center;justify-content: center;border-radius: 10%;border-color: #ffce33;border: 1px;"}
<!-- .box-D1 -->
Hidden$\neq$ Disappeared
:::
...
:::{.absolute top=145px left=40px width=105px height=75px style="background-color:#cceffc; font-weight:bold; color:#000; text-align:left; z-index:999;font-size:80%!important;border-radius:5%;"}
<!-- .box-Ds5 -->
D:
:::
...
:::{.absolute top=480px left=105px width=110px height=75px style="background-color:#cceffc; font-weight:bold; color:#000; text-align:left; z-index:999;font-size:80%!important;"}
<!-- .box-Ds4 -->
S:
:::
...
::: {.notes}
And the instrumental variable $Z$ should satisfies both relevance condition and exogeneity condition as we have mentioned before.So, what are these appropriate instruments for education?
:::
...
## Weak instrument: Birth weight example
...
## Weak instrument test (solution 1): Restricted F-test

<!-- some pictures -->

##

### Test situation: Only one endogenous regressor in the model
...
## Weak instrument test (solution 1): Restricted F-test on wage example 
...
## Weak instrument test (solution 2): Cragg-Donald test

<!-- some pictures -->

##

### Test situation: More than one endogenous regressor in the model
...
## Weak instrument test (solution 2): Crag-Donald test on Work hour example
...
## Instrument Exogeneity: Introduction
...
## Instrument Exogeneity test: the J-test solution

<!-- some pictures -->

##

### Procedure of J-test  (1/2)
...
## Instrument Exogeneity test: the J-test solution on wage example
...
# 17.6 Testing Regressor endogeneity{#test-endogeneity .monash-bg-blue .mcenter}
...
## Regressor Endogeneity: Introduction
...
## Test regressor endogeneity: the Hausman test solution
...
## Test regressor endogeneity: the Hausman test on Wage example
...
# chapter Summary {#smry .monash-bg-blue .mcenter}
