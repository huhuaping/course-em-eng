...
source(here::here("R","create-DTbutton.R"))
```
...
cat(
  paste0(
    c("---\n\n",
      "exclude:true",
      "\n\n",
      "### Source A：Omitted variables (demo1)"), 
    "\n\n",
    "Here we show a visual demonstration on this situation:
\n",
    "\n",
    paste0("![](pic/chpt11-endogeneity-ommit-sequence/chpt11-ommit-sequence-0",4:1,".png)"),
    "\n\n",
    "---\n\n",
  sep = ""),
sep = "")
...
<!---demo2 ommit endo begin--->

```{r, results='asis'}
cat(
  paste0(
    c("### Source 1：Omitted variables  (demo2)"), 
    "\n\n",
    "An intuitive demonstration is show as follows:\n",
    "\n",
    paste0("![](pic/chpt11-endogeneity-ommit-sequence/chpt11-ommit-sequence-part-0",5:1,".png)"),
    "\n\n",
    "---\n\n",
  sep = ""),
sep = "")
...
cat(
  paste0(
    c("### Source 2: Measurement errors (demo 1)"), 
    "\n\n",
    "Here we show a visual demonstration on this situation: \n",
    "\n",
    paste0("![](pic/chpt11-endogeneity-measure-sequence/chpt11-measure-sequence-0",4:1,".png)"),
    "\n\n",
    "---\n\n",
  sep = ""),
sep = "")
...
cat(
  paste0(
    c("### Source 2: Measurement errors (demo 2)"), 
    "\n\n",
    "An intuitive demonstration is show as follows:\n",
    "\n",
    paste0("![](pic/chpt11-endogeneity-measure-sequence/chpt11-measure-sequence-part-0",6:1,".png)"),
    "\n\n",
    "---\n\n",
  sep = ""),
sep = "")
...
cat(
  paste0(
    c("### Source 3: Autocorrelation (demo 1)"), 
    "\n\n",
    "Here we show a visual demonstration on this situation:\n",
    "\n",
    paste0("![](pic/chpt11-endogeneity-ar-sequence/chpt11-ar-sequence-0",4:1,".png)"),
    "\n\n",
    "---\n\n",
  sep = ""),
sep = "")
...
cat(
  paste0(
    c("### Source 3: Autocorrelation (demo 2)"), 
    "\n\n",
    "An intuitive demonstration is show as follows:\n",
    "\n",
    paste0("![](pic/chpt11-endogeneity-ar-sequence/chpt11-ar-sequence-part2-0",5:1,".png)"),
    "\n\n",
    "---\n\n",
  sep = ""),
sep = "")
...
cat(
  paste0(
    c("### Source 4: Simultaneity (demo 1)"), 
    "\n\n",
    "Here we show a visual demonstration on this situation:\n",
    "\n",
    paste0("![](pic/chpt11-endogeneity-sem-sequence/chpt11-sem-sequence-0",4:1,".png)"),
    "\n\n",
    "---\n\n",
  sep = ""),
sep = "")
...
- In the theory analysis, we have known that this model is mis-specified due to the important omitted variables(`ability`) and it will be dropped to the disturbance term 
$u_i$. 

- While the regressor `edu` is correlated with the disturbance term 
$u_i$ (formally the omitted variables `ability`), thus the regressor `edu` is endogenous variable!
...
```{r,results='asis'}
fun_report_eq(lm.mod = mod_origin, lm.dt = mroz, lm.n = 4)
```
...
```{r, results="asis"}
out <- lx.est(
  lm.mod = mod_step1, lm.n = 4,
  lm.dt = mroz, inf = c("fit", "Ftest")
  )
```
...
```{r, results="asis"}
out <- lx.est(
  lm.mod = mod_step2, lm.n = 4,
  lm.dt = mroz_add, inf = c("fit", "Ftest")
  )
```
...
tab_smry<-as_tibble(smry.system_m$coefficients)
...
```{r, ref.label='systemfit-m',echo=TRUE}
...
tab_smry<-as_tibble(smry.system_f$coefficients)
names(tab_smry)<-cnames
tab_smry$vars<-rnames
tab_smry <- tab_smry %>%
  separate(vars,into=c("eq","vars"),sep="_")%>%
...
```{r, ref.label='systemfit-f',echo=TRUE}
...
tab_smry<-as_tibble(smry.system_mf$coefficients)
names(tab_smry)<-cnames
tab_smry$vars<-rnames
tab_smry <- tab_smry %>%
  separate(vars,into=c("eq","vars"),sep="_")%>%
...
```{r, ref.label='systemfit-mf',echo=TRUE}
...
```{r, ref.label='ivreg-mf',echo=TRUE}
...
```{r}
include_graphics("pic/chpt17-iv-comparison.png", dpi=180)
```
...
library("stargazer")
stargazer(ols_origin, ols_step2, lm_iv_m, lm_iv_f, lm_iv_mf,
...
> Note: **Restriced F test** (`r number(F_r1, 0.01)`) is different with the **classical OLS F test**(show bellow `r number(F_c1, 0.01)`).
...
kable(out, align = "c", caption = "Cragg-Donald test results")
```
...
```{r, ref.label="ivreg-mf", eval=FALSE, echo=TRUE}
...
```{r}
# show restricted F-test summary
restricted_ftest
```
...
The restricted F-statistics is `r number(restricted_f, 0.0001)` (with round digits 4 here ).
...
- The calculated J-statistic is `r number(jtest_calc, 0.0001)` (with round digits 4 here ).
...
- We obtain the J-statistic `r number(jtest_chi, 0.0001)` (with round digits 4 here ). It's the same as what we have calculated by hand!
...
${\chi^2}^{\ast} =`r number(jtest_chi, 0.0001)`$, and its correct freedom is $(m-k)=1$.
